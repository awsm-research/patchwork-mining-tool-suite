...
class MailingListFormat1Spider(scrapy.Spider):
    ...
    def __init__(self, oid=1, ecosystem="ffmpeg", *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.original_id = int(oid)
    ...
    def parse_email(self, response):
        ...
        if self.base_func.is_patch_related(email_subject, email_raw_content):
            ...
            email_msg_id = re.search(
                r'In-Reply-To=(<.*?>)', decoded_info).group(1)
            email_date = self.base_func.convert_datetime(
                response.xpath("//body/i/text()").get())
            sender_name = response.xpath("//body/b/text()").get()
            web_url = response._url
            project_original_id = self.base_func.get_project_original_id(
                web_url)

            mailing_list_item = MailingListItem(
                original_id=f"{self.ecosystem}-mailinglist-{self.original_id}",
                msg_id=email_msg_id,
