...
class ProcessInitialData():
    ...
    def group_patches_step1(self, patch_data):
        ...
        while i < len(sorted_patch_data):
            ...
            while j < len(sorted_patch_data):
                ...
                token_diff = (one_gram_i | one_gram_j) - \
                    (one_gram_i & one_gram_j)
                ...
                if self.__step1_conditions(token_diff, newseries_original_id_i, newseries_original_id_j, series_original_id_i, series_original_id_j):
                    ...
                    step1_groups[target_group_idx]['versions'].append(
                        version_j)
                    step1_groups[target_group_idx]['submitters'].append(
                        submitter_j)
                    step1_groups[target_group_idx]['individuals'].append(
                        individual_j)
                    step1_groups[target_group_idx]['series_original_ids'].append(
                        series_original_id_j)
                    step1_groups[target_group_idx]['newseries_original_ids'].append(
                        newseries_original_id_j)
                    step1_groups[target_group_idx]['state'].append(state_j)
                    step1_groups[target_group_idx]['commit_id'].append(
                        commit_id_j)
    ...
    def group_patches_step2(self, patch_data: list):
        ...
        sorted_patch_data, step1_groups, change1_collection = self.group_patches_step1(
            patch_data)
        ...
        for curr_len in token_lengths:
            ...
            next_groups = deepcopy(
                group_data[curr_len + 1]) if (curr_len + 1) in token_lengths else list()
            ...
            for group_i in curr_groups:
                if curr_groups.index(group_i) % 500 == 0:
                    et = time.time()
                    print(
                        f"token_length {curr_len} - group {curr_groups.index(group_i)} - time: {(et - st) / 60: .2f} min, total: {(et - base_st) / 60: .2f} min")
                    st = time.time()
                ...
                if next_groups and curr_len > 0:
                    next_groups = sorted(
                        next_groups, key=lambda x: x['tokens'])
                    ...
                    while j < len(next_groups):
                        ...
                        if group_i['tokens'][0] == group_j['tokens'][0]:
                            ...
                            if self.__step2_conditions(individual_i, individual_j, token_diff, newseries_original_id_i, newseries_original_id_j, series_original_id_i, series_original_id_j, versions_i, versions_j):
                                ...
                                step2_groups[target_idx]['group'].extend(
                                    group_j['group'])
                                step2_groups[target_idx]['versions'].extend(
                                    versions_j)
                                step2_groups[target_idx]['series_original_ids'].extend(
                                    series_original_id_j)
                                step2_groups[target_idx]['newseries_original_ids'].extend(
                                    newseries_original_id_j)
                                step2_groups[target_idx]['one_gram'].append(
                                    one_gram_j)
                                step2_groups[target_idx]['tokens'].append(
                                    tokens_j)

                                step2_groups[target_idx]['state'].extend(
                                    group_j['state'])
                                step2_groups[target_idx]['commit_id'].extend(
                                    group_j['commit_id'])
                                step2_groups[target_idx]['submitters'].extend(
                                    group_j['submitters'])
                                step2_groups[target_idx]['individuals'].extend(
                                    group_j['individuals'])
                    ...
                    next_groups = sorted(
                        next_groups, key=lambda x: x['tokens2'])
    ...
    def __sort_patches(self, patches):
        for i in range(len(patches)):
            patch_i = patches[i]
            patch_i['tokens'], patch_i['one_gram'] = self.__create_one_gram(
                patch_i['name'])
